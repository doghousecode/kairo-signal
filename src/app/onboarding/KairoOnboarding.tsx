...
const SUBTOPICS = {
  ...
  politics: [
    { id: "uk_pol", label: "UK", Icon: Landmark },
    { id: "us_pol", label: "US", Icon: Vote },
    { id: "geopolitics", label: "Global", Icon: Globe },
  ...
  automotive: [
    ...
    { id: "mods", label: "Tuning", Icon: Wrench },
...
const EXPANDED_R = 54;
const SUB_R_BASE = 24;
const SUB_R_SELECTED = 28;
const SUB_R_BOOSTED = 33;
const SUB_ORBIT = 82;
...
const COLLISION_RESTITUTION = 0.88;
const FLING_MULTIPLIER = 5.5;
...
function initPositions(count, w, h) {
  const out = [];
  const maxR = BASE_R + 25, spacing = maxR * 2.4;
  ...
  for (let i = 0; i < count; i++) {
    ...
    out.push({ x: Math.max(maxR, Math.min(w - maxR, startX + col * spacing + Math.sin(i * 3.7) * spacing * 0.12)), y: Math.max(maxR, Math.min(h - maxR, startY + row * spacing + Math.cos(i * 2.3) * spacing * 0.12)), vx: 0, vy: 0, dpx: Math.random() * Math.PI * 2, dpy: Math.random() * Math.PI * 2, dax: DRIFT_AMP * (0.7 + Math.random() * 0.6), day: DRIFT_AMP * (0.7 + Math.random() * 0.6), dsx: DRIFT_SPEED * (0.6 + Math.random() * 0.8), dsy: DRIFT_SPEED * (0.5 + Math.random() * 0.9) });
  }
  return out;
}

function usePhysics(topics, selected, cSize, expandedId) {
  const bodies = useRef([]); const anim = useRef(null); const drag = useRef({ idx: -1, mx: 0, my: 0, pmx: 0, pmy: 0 }); const [pos, setPos] = useState([]); const init = useRef(false);
  useEffect(() => { if (cSize.w < 100 || cSize.h < 100) return; if (init.current && bodies.current.length === topics.length) return; bodies.current = initPositions(topics.length, cSize.w, cSize.h); init.current = true; setPos(bodies.current.map(b => ({ x: b.x, y: b.y }))); }, [topics.length, cSize.w, cSize.h]);
  const gr = useCallback((i) => { const t = topics[i], s = selected[t?.id]; return getR(s !== undefined, s ?? -1, expandedId === t?.id); }, [selected, topics, expandedId]);
  const cancelDrag = useCallback(() => { if (drag.current.idx >= 0) { const b = bodies.current[drag.current.idx]; if (b) { b.vx = 0; b.vy = 0; } } drag.current.idx = -1; }, []);
  useEffect(() => {
    ...
    const step = (ts) => {
      ...
      for (let i = 0; i < n; i++) {
        const b = B[i], ri = gr(i);
        if (d.idx === i) { b.vx = (d.mx - b.x) * 0.4; b.vy = (d.my - b.y) * 0.4; b.x += b.vx; b.y += b.vy; } else {
          let fx = 0, fy = 0;
          for (let j = 0; j < n; j++) { if (i === j) continue; const rj = gr(j), dx = b.x - B[j].x, dy = b.y - B[j].y, dist = Math.sqrt(dx * dx + dy * dy), minD = ri + rj + 4; if (dist < minD && dist > 0.5) { const nx = dx / dist, ny = dy / dist, dvx = b.vx - B[j].vx, dvy = b.vy - B[j].vy, dvN = dvx * nx + dvy * ny; if (dvN < 0) { const imp = dvN * COLLISION_RESTITUTION; if (d.idx !== i) { b.vx -= imp * nx; b.vy -= imp * ny; } if (d.idx !== j) { B[j].vx += imp * nx; B[j].vy += imp * ny; } } const ol = minD - dist, sep = ol * 0.5; if (d.idx !== i) { b.x += nx * sep * 0.5; b.y += ny * sep * 0.5; } if (d.idx !== j) { B[j].x -= nx * sep * 0.5; B[j].y -= ny * sep * 0.5; } } }
          const m = ri + 4;
          if (b.x <= m) { b.x = m + 0.1; b.vx = Math.abs(b.vx) * WALL_BOUNCE; } else if (b.x >= w - m) { b.x = w - m - 0.1; b.vx = -Math.abs(b.vx) * WALL_BOUNCE; }
          if (b.y <= m) { b.y = m + 0.1; b.vy = Math.abs(b.vy) * WALL_BOUNCE; } else if (b.y >= h - m) { b.y = h - m - 0.1; b.vy = -Math.abs(b.vy) * WALL_BOUNCE; }
          const dcx = w / 2 - b.x, dcy = h / 2 - b.y; if (Math.sqrt(dcx * dcx + dcy * dcy) > Math.min(w, h) * 0.42) { fx += dcx * 0.00006; fy += dcy * 0.00006; }
          const speed = Math.sqrt(b.vx * b.vx + b.vy * b.vy); if (speed < 1.5) { fx += Math.sin(ts * b.dsx + b.dpx) * b.dax * 0.002; fy += Math.cos(ts * b.dsy + b.dpy) * b.day * 0.002; }
          b.vx = clampV((b.vx + fx) * DAMPING); b.vy = clampV((b.vy + fy) * DAMPING); b.x += b.vx; b.y += b.vy; b.x = Math.max(m, Math.min(w - m, b.x)); b.y = Math.max(m, Math.min(h - m, b.y));
        }
      }
      setPos(B.map(b => ({ x: b.x, y: b.y }))); anim.current = requestAnimationFrame(step);
    };
  ...
  const endDrag = () => { if (drag.current.idx >= 0) { const b = bodies.current[drag.current.idx]; if (b) { b.vx = clampV((drag.current.mx - drag.current.pmx) * FLING_MULTIPLIER); b.vy = clampV((drag.current.my - drag.current.pmy) * FLING_MULTIPLIER); } } drag.current.idx = -1; };
  const nudge = (idx) => { const B = bodies.current, b = B[idx]; if (!b) return; for (let j = 0; j < B.length; j++) { if (j === idx) continue; const dx = B[j].x - b.x, dy = B[j].y - b.y, dist = Math.sqrt(dx * dx + dy * dy) || 1; B[j].vx = clampV(B[j].vx + (dx / dist) * 4); B[j].vy = clampV(B[j].vy + (dy / dist) * 4); } };
  return { pos, startDrag, moveDrag, endDrag, nudge, cancelDrag };
}

function GlassOrb({ topic, isSelected, priority, pos, onPointerDown, onClick, isDragging, isExpanded, children }) {
  const r = getR(isSelected, priority, isExpanded), sz = r * 2, IC = topic.Icon;
  const iconSize = Math.round(r * (isExpanded ? 0.4 : 0.55)), labelSize = Math.max(11, Math.round(r * (isExpanded ? 0.2 : 0.28)));
  return (
    <div className="absolute select-none touch-none" style={{ left: pos.x - r, top: pos.y - r, width: sz, height: sz, zIndex: isDragging ? 100 : isExpanded ? 50 : isSelected ? 10 : 1, transition: isDragging ? "none" : "width 0.5s cubic-bezier(0.34,1.56,0.64,1), height 0.5s cubic-bezier(0.34,1.56,0.64,1)" }}>
      {children}
      <div onPointerDown={onPointerDown} onClick={onClick} className="w-full h-full rounded-full flex flex-col items-center justify-center relative overflow-hidden" style={{ cursor: isDragging ? "grabbing" : "grab", background: isSelected ? `radial-gradient(circle at 35% 35%, ${topic.color}55, ${topic.color}30 60%, ${topic.color}18 100%)` : "radial-gradient(circle at 35% 35%, rgba(255,255,255,0.08), rgba(255,255,255,0.02) 60%, rgba(255,255,255,0.01) 100%)", boxShadow: isSelected ? `inset 0 0 ${r * 0.6}px ${topic.color}20, 0 0 ${r * 0.8}px ${topic.color}18, 0 0 ${r * 1.5}px ${topic.color}08, 0 4px 20px rgba(0,0,0,0.15)` : "inset 0 0 15px rgba(255,255,255,0.04), 0 4px 20px rgba(0,0,0,0.06)", border: isSelected ? `1.5px solid ${topic.color}60` : "1px solid rgba(255,255,255,0.12)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", transition: isDragging ? "background 0.3s, box-shadow 0.3s, border 0.3s" : "all 0.5s cubic-bezier(0.34,1.56,0.64,1)" }}>
        <div className="absolute pointer-events-none" style={{ top: "6%", left: "12%", width: "45%", height: "35%", background: isSelected ? "linear-gradient(170deg, rgba(255,255,255,0.5) 0%, rgba(255,255,255,0.08) 50%, transparent 100%)" : "linear-gradient(170deg, rgba(255,255,255,0.25) 0%, rgba(255,255,255,0.03) 50%, transparent 100%)", borderRadius: "50%", filter: "blur(2px)" }} />
        <div className="absolute pointer-events-none" style={{ top: "14%", left: "22%", width: "12%", height: "8%", background: isSelected ? "rgba(255,255,255,0.7)" : "rgba(255,255,255,0.2)", borderRadius: "50%", filter: "blur(2px)" }} />
        <div className="absolute pointer-events-none" style={{ bottom: "3%", left: "15%", width: "70%", height: "25%", background: isSelected ? `linear-gradient(90deg, ${topic.color}30, rgba(255,180,255,0.15), rgba(180,255,255,0.15), ${topic.color}20)` : "linear-gradient(90deg, rgba(255,255,255,0.03), rgba(200,180,255,0.05), rgba(180,255,230,0.05), rgba(255,255,255,0.03))", borderRadius: "50%", filter: "blur(4px)" }} />
        <div className="absolute inset-0 rounded-full pointer-events-none" style={{ background: isSelected ? `radial-gradient(circle, transparent 55%, ${topic.color}12 75%, ${topic.color}06 90%, transparent 100%)` : "radial-gradient(circle, transparent 60%, rgba(255,255,255,0.04) 80%, transparent 100%)" }} />
        ...
        <span className="mt-1 font-bold tracking-tight text-center" style={{ fontSize: labelSize, color: isSelected ? "#fff" : "rgba(255,255,255,0.35)", fontFamily: "'Outfit', sans-serif", textShadow: isSelected ? `0 1px 4px ${topic.color}60` : "none", transition: "all 0.3s ease" }}>{topic.label}</span>
        {isSelected && priority >= 0 && !isExpanded && (<div className="flex gap-1 mt-0.5">{[...Array(priority + 1)].map((_, i) => <div key={i} className="rounded-full" style={{ width: 4, height: 4, background: "rgba(255,255,255,0.7)" }} />)}</div>)}
...
function SubOrb({ sub, parentColor, angle, priority, onClick, visible }) {
  const IC = sub.Icon;
  const isSelected = priority >= 0;
  const isBoosted = priority === 1;
  const r = getSubR(priority);
  const x = Math.cos(angle) * SUB_ORBIT, y = Math.sin(angle) * SUB_ORBIT;
  const iconSize = isBoosted ? 17 : isSelected ? 15 : 13;
  const fontSize = isBoosted ? 9 : 8;
  ...
  return (
    <div onClick={e => { e.stopPropagation(); onClick(); }}
      ...
      style={{
        ...
        left: `calc(50% + ${x}px - ${r}px)`, top: `calc(50% + ${y}px - ${r}px)`,
        background: isBoosted
          ? `radial-gradient(circle at 35% 35%, ${parentColor}aa, ${parentColor}70)`
          : isSelected
          ? `radial-gradient(circle at 35% 35%, ${parentColor}77, ${parentColor}44)`
          : "radial-gradient(circle at 35% 35%, rgba(255,255,255,0.1), rgba(255,255,255,0.03))",
        border: isBoosted
          ? `2px solid ${parentColor}aa`
          : isSelected
          ? `1.5px solid ${parentColor}70`
          : "1px solid rgba(255,255,255,0.15)",
        boxShadow: isBoosted
          ? `0 0 18px ${parentColor}35, 0 0 6px ${parentColor}20, 0 2px 10px rgba(0,0,0,0.2)`
          : isSelected
          ? `0 0 12px ${parentColor}20, 0 2px 8px rgba(0,0,0,0.15)`
          : "0 2px 8px rgba(0,0,0,0.08)",
        backdropFilter: "blur(10px)", WebkitBackdropFilter: "blur(10px)",
        opacity: visible ? 1 : 0, transform: visible ? "scale(1)" : "scale(0.3)",
        transition: "all 0.4s cubic-bezier(0.34, 1.56, 0.64, 1)",
        transitionDelay: visible ? `${Math.random() * 0.15}s` : "0s", zIndex: 60,
      }}>
      <div className="absolute pointer-events-none rounded-full" style={{ top: "8%", left: "15%", width: "40%", height: "28%", background: isSelected ? "linear-gradient(170deg, rgba(255,255,255,0.4) 0%, transparent 100%)" : "linear-gradient(170deg, rgba(255,255,255,0.25) 0%, transparent 100%)", filter: "blur(1.5px)" }} />
      <IC size={iconSize} strokeWidth={isBoosted ? 2.5 : isSelected ? 2 : 1.5} style={{ color: isSelected ? "#fff" : "rgba(255,255,255,0.5)", filter: isBoosted ? `drop-shadow(0 1px 2px ${parentColor}80)` : "none", transition: "all 0.3s ease" }} />
      <span className="font-semibold text-center leading-tight" style={{ fontSize, color: isSelected ? "#fff" : "rgba(255,255,255,0.4)", fontFamily: "'Outfit', sans-serif", marginTop: 1, textShadow: isBoosted ? `0 1px 3px ${parentColor}50` : "none", transition: "all 0.3s ease" }}>{sub.label}</span>
      {isSelected && (
        <div className="flex gap-0.5 mt-0.5">
          {[...Array(priority + 1)].map((_, i) => <div key={i} className="rounded-full" style={{ width: 3, height: 3, background: "rgba(255,255,255,0.7)" }} />)}
        </div>
      )}
    </div>
  );
...
export default function KairoOnboarding() {
  ...
  // subSelected: { topicId: { subId: 0|1 } } — 0 = selected, 1 = boosted
  ...
  const { pos, startDrag, moveDrag, endDrag, nudge, cancelDrag } = usePhysics(TOPICS, selected, cSize, expandedId);
  ...
  useEffect(() => {
    const release = () => { if (dragIdx >= 0) { cancelDrag(); setDragIdx(-1); if (longPressTimer.current) { clearTimeout(longPressTimer.current); longPressTimer.current = null; } if (activeListeners.current) { window.removeEventListener("pointermove", activeListeners.current.move); window.removeEventListener("pointerup", activeListeners.current.up); activeListeners.current = null; } } };
    document.addEventListener("pointerleave", release); document.addEventListener("pointercancel", release); window.addEventListener("blur", release);
    return () => { document.removeEventListener("pointerleave", release); document.removeEventListener("pointercancel", release); window.removeEventListener("blur", release); };
  }, [dragIdx, cancelDrag]);

  const handlePD = (i, e) => {
    e.preventDefault(); startPosRef.current = { x: e.clientX, y: e.clientY }; wasDragRef.current = false; longPressTriggered.current = false;
    ...
    if (selected[topicId] !== undefined && SUBTOPICS[topicId]) { longPressTimer.current = setTimeout(() => { if (!wasDragRef.current) { longPressTriggered.current = true; setExpandedId(prev => prev === topicId ? null : topicId); } }, LONG_PRESS_MS); }
    const onMove = ev => { ev.preventDefault(); const cx = ev.touches ? ev.touches[0].clientX : ev.clientX, cy = ev.touches ? ev.touches[0].clientY : ev.clientY; if (Math.abs(cx - startPosRef.current.x) > 5 || Math.abs(cy - startPosRef.current.y) > 5) { wasDragRef.current = true; if (longPressTimer.current) { clearTimeout(longPressTimer.current); longPressTimer.current = null; } } moveDrag(cx, cy, rect); };
    const onUp = () => { if (longPressTimer.current) { clearTimeout(longPressTimer.current); longPressTimer.current = null; } endDrag(); setDragIdx(-1); window.removeEventListener("pointermove", onMove); window.removeEventListener("pointerup", onUp); activeListeners.current = null; };
    window.addEventListener("pointermove", onMove); window.addEventListener("pointerup", onUp); activeListeners.current = { move: onMove, up: onUp };
  };
  ...
  const handleBgClick = () => { if (longPressTriggered.current) { longPressTriggered.current = false; return; } if (expandedId) setExpandedId(null); };

  // Sub-topic toggle: undefined -> 0 (selected) -> 1 (boosted) -> undefined (removed)
  const toggleSub = (topicId, subId) => {
    setSubSelected(prev => {
      const topicMap = { ...(prev[topicId] || {}) };
      const current = topicMap[subId];
      if (current === undefined) {
        topicMap[subId] = 0;
      } else if (current === 0) {
        topicMap[subId] = 1;
      } else {
        delete topicMap[subId];
      }
  ...
  const getSubPriority = (topicId, subId) => {
    const map = subSelected[topicId];
    if (!map) return -1;
    const val = map[subId];
    return val !== undefined ? val : -1;
  };

  const countSubs = (topicId) => {
    const map = subSelected[topicId];
  ...
  const totalSubs = Object.values(subSelected).reduce((n, m) => n + Object.keys(m).length, 0);
        ...
        {phase === 0 && (
          <div className={`flex-1 flex flex-col ${entering ? "k-phase-in" : ""}`}>
            <div className="text-center mt-1 mb-0">
              ...
              <p className="text-white/30 text-base">Tap to select. Tap again to boost. Long-press to explore. Drag to throw.</p>
            </div>
            <div ref={cRef} className="flex-1 relative w-full overflow-hidden mt-2" style={{ touchAction: "none" }} onClick={handleBgClick}>
              {pos.length > 0 && TOPICS.map((t, i) => {
                const isSel = selected[t.id] !== undefined, isExp = expandedId === t.id;
                const subs = SUBTOPICS[t.id] || [];
                return (
                  <GlassOrb key={t.id} topic={t} isSelected={isSel} priority={selected[t.id] ?? -1} pos={pos[i] || { x: 0, y: 0 }} isDragging={dragIdx === i} isExpanded={isExp} onPointerDown={e => handlePD(i, e)} onClick={() => handleClick(i)}>
                    {isSel && subs.map((sub, si) => {
                      const angle = (si / subs.length) * Math.PI * 2 - Math.PI / 2;
                      return <SubOrb key={sub.id} sub={sub} parentColor={t.color} angle={angle} priority={getSubPriority(t.id, sub.id)} visible={isExp} onClick={() => toggleSub(t.id, sub.id)} />;
                    })}
                  </GlassOrb>
                );
              })}
            ...
            {selTopics.length > 0 && (
              <div className="text-center pb-2" style={{ animation: "kFadeIn 0.4s ease forwards" }}>
                <div className="flex items-center justify-center gap-2 flex-wrap">
                  {selTopics.map(t => { const IC = t.Icon; const sc = countSubs(t.id); return (
                    <span key={t.id} className="px-3 py-1 rounded-full text-xs font-semibold inline-flex items-center gap-1.5" style={{ background: `${t.color}12`, color: t.color, border: `1px solid ${t.color}25`, fontFamily: "'Outfit', sans-serif" }}>
                      <IC size={12} />{t.label}{sc > 0 && <span className="opacity-60">+{sc}</span>}<span className="opacity-40">{["\u25CB", "\u25D1", "\u25CF"][selected[t.id] ?? 0]}</span>
                    </span>
                  ); })}
                </div>
              </div>
